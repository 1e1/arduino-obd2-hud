...
const bool Hudisplay::requestAnimationFrame(const bool force)
{
    const unsigned long now = millis();

    if (!force && (now < this->_nextFrameTimeMs)) {
        return false;
    }

    this->_nextFrameTimeMs = now + HUDISPLAY_FRAME_DURATION_MS;
    ...
    if (this->_frameIndex == HUDISPLAY_HISTORY_FRAME_SIZE) {
        this->_frameIndex = 0;
    }
...
const uint8_t Hudisplay::_getTankCapacity(void)
{
    return this->_currentTankCapacity;
}


const uint8_t Hudisplay::_getTankLoad(void)
{
    return this->_currentTankLoad;
}


const unsigned long Hudisplay::_getOdometerValue(void)
{
    return this->_currentOdometerValue;
}


const uint8_t Hudisplay::_getRpmValueInDiv64(void)
{
    return this->_rpmList[this->_frameIndex];
}


const uint8_t Hudisplay::_getSpeedValue(void)
{
    return this->_speedList[this->_frameIndex];
}


const uint8_t Hudisplay::_getFuelConsumptionValue(void)
{
...
const uint8_t Hudisplay::_retrieveMin(MinMax* minMax)
{
    uint8_t min = minMax[0].min;

    for (uint8_t i=1; i<HUDISPLAY_HISTORY_FRAME_SIZE; ++i) {
        if (min > minMax[i].min) {
            min = minMax[i].min;
        }
    }
...
const uint8_t Hudisplay::_retrieveMin(uint8_t* valueList)
{
...
const uint8_t Hudisplay::_retrieveMax(MinMax* minMax)
{
...
const uint8_t Hudisplay::_retrieveMax(uint8_t* valueList)
{
...
const unsigned long Hudisplay::_getDistance(void)
{
    return this->_currentOdometerValue - this->_firstOdometerValue;
}


const unsigned short Hudisplay::_millisIn4s(void)
{
    return int(millis() >> 12);
}


const unsigned short Hudisplay::_getDuration(void)
{
    return this->_millisIn4s() - this->_firstMillisIn4s;
}


const uint8_t Hudisplay::_getTankValue(void)
{
    return this->_fuelLoadToL(this->_getTankLoad());
}


const uint8_t Hudisplay::_getConsumptionLoad(void)
{
    return this->_firstTankLoad - this->_currentTankLoad;
}


const uint8_t Hudisplay::_getConsumptionInL(void)
{
    return this->_fuelLoadToL(this->_getConsumptionLoad());
}


const uint8_t Hudisplay::_fuelLoadToL(const uint8_t load)
{
    return (load * this->_getTankCapacity()) >> 8;
}


const float Hudisplay::_getAverageConsumptionInDlp100km(void)
{
...
const uint8_t Hudisplay::_getAverageSpeedInKmph(void)
{
    const unsigned short lengthInHm = this->_getDistance();
